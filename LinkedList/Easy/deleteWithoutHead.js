...
// Difficulty: EasyAccuracy: 78.57%Submissions: 210K+Points: 2
...
function deleteNode(del_node) {
    if (!del_node || !del_node.next) {  // if(!del_node){}
...
let del_node = head; // Node to be deleted

deleteNode(del_node); // Deleting node with value 1

// Output the updated list
let temp = head;
if (temp.value === 1 && temp.next === null) {
    temp = temp.next; // Ensure we correctly handle edge cases
}
let result = [];
while (temp) {
